  ...
  getURLs = async () => {
    ...
    Object.keys(data).map(each => {
      this.setState(prevState => ({
        currentURL: (prevState.currentUrls[data[each].hash] =
          data[each].originalUrl)
      }));
    });
  };
  ...
  submitHandler = e => {
    ...
    if (newUrl === "") {
      ...
      if (Object.values(currentUrls).includes(newUrl)) {
        const link = Object.keys(currentUrls).find(
          url => currentUrls[url] === newUrl
        );
        this.setState({
          openModal: true,
      ...
      <div className={appStyle.App}>
        ...
        <div className={appStyle.positioning}>
          <form className={appStyle.box} onSubmit={e => this.submitHandler(e)}>
            ...
            <div>
              <button type="submit" className={appStyle.inputSubmit}>
                Submit
              </button>
            </div>
          </form>
